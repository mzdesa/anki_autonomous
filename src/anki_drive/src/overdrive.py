...
class Overdrive:
    ...
    def connect(self):
        ...
        self._readChar = self._peripheral.getCharacteristics(
            1, 0xFFFF, "be15bee06186407e83810bd89c4d8df4")[0]
        self._writeChar = self._peripheral.getCharacteristics(
            1, 0xFFFF, "be15bee16186407e83810bd89c4d8df4")[0]
    ...
    def enableNotify(self):
        ...
        while True:
            ...
            self._peripheral.writeCharacteristic(
                self._readChar.valHandle + 1, b"\x01\x00")
            self.ping()
    ...
    def _locationChangeCallback(self, location, piece, speed, clockwise):
        ...
        if self._locationChangeCallbackFunc is not None:
            self._locationChangeCallbackFunc(
                self.addr, location, piece, speed, clockwise)
        ...
        if self.handle == handle:
            ...
            if commandId == 0x27:
                ...
                if clockwiseVal == 0x47:
                    clockwise = True
                threading.Thread(target=self.overdrive._locationChangeCallback, args=(
                    location, piece, speed, clockwise)).start()
            if commandId == 0x29:
                # Transition notification
                piece, piecePrev, offset, direction = struct.unpack_from(
                    "<BBfB", data, 2)
                threading.Thread(
                    target=self.overdrive._transitionCallback).start()
            elif commandId == 0x17:
                # Pong
