...
class ImgService:
  ...
  def __init__(self):
    ...
    # rospy.Subscriber("/camera/color/image_raw", Image, self.imgReceived)
    rospy.Subscriber("/usb_cam/image_raw", Image, self.imgReceived)

    #Create the service
    rospy.Service('/last_color_image', ImageSrv, self.getLastImage)

  def run(self):
    rospy.loginfo('Running color image server ...')
    rospy.spin()
